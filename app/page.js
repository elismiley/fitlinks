...
  return (
    <div style={{width:size,height:size,borderRadius:"50%",background:isYou?"rgba(196,160,90,0.2)":"rgba(255,255,255,0.05)",border:`1.5px solid ${isYou?"#C4A05A":"rgba(255,255,255,0.1)"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.33,fontWeight:700,color:isYou?"#C4A05A":"#888",flexShrink:0}}>
      {initials}
    </div>
  );
}
...
  const min = Math.min(...scores)-0.5, max = Math.max(...scores)+0.5, range = max-min||1;
  const w=120, h=40;
...
      {scores.map((s,i)=>{
        const x=(i/(scores.length-1))*w, y=h-((s-min)/range)*h;
...
            <div style={{fontSize:"0.6rem",color:"#444"}}>mod {mod.toFixed(2)}x</div>
          </div>
        </div>
...
      <div style={{fontSize:"1.2rem",color:"#666",marginBottom:"2rem"}}>{hole.reps?`${hole.reps} reps`:isHold?"Hold as long as possible":"Complete the distance"}</div>
...
      </div>
      <button onClick={done} style={{width:"100%",maxWidth:380,padding:"2rem 1.1rem",background:"#C4A05A",border:"none",color:"#0F0F0E",fontSize:"1.3rem",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",borderRadius:"8px",cursor:"pointer"}}>Complete</button>
    </div>
  );
}
...
        <div style={{fontSize:"0.65rem",color:"#444",marginTop:"0.4rem"}}>Movement {fmtScore(sd?.mov)} x Rest {sd?.mod?.toFixed(2)}x</div>
...
      <div style={{fontSize:"2rem",marginBottom:"1rem"}}>golf</div>
...
function ClubOnboarding({ user, onDone }) {
...
          createdAt: Date.now(),
        }),
        db.tx.profiles[user.id].update({ clubId }),
      ]);
...
    setLoading(true); setError("");
    try {
      const { data } = await db.queryOnce({ clubs: {} });
      const clubsArr = Object.values(data.clubs || {});
      const club = clubsArr.find(c => c.inviteCode === inviteCode.trim().toUpperCase());
      if (!club) { setError("Invalid invite code"); setLoading(false); return; }
      if ((club.memberIds || []).length >= 6) { setError("Club is full (6/6)"); setLoading(false); return; }
      const newMembers = [...(club.memberIds || []), user.id];
      await db.transact([
        db.tx.clubs[club.id].update({ memberIds: newMembers }),
        db.tx.profiles[user.id].update({ clubId: club.id }),
      ]);
      onDone();
    } catch (e) {
      setError("Error: " + e.message);
...
  const s = {
    wrap: { minHeight:"100vh", background:"#0F0F0E", color:"#E8E0D0", fontFamily:"Georgia,serif", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"2rem" },
    title: { fontSize:"1.5rem", fontWeight:700, marginBottom:"0.5rem", textAlign:"center" },
    sub: { fontSize:"0.75rem", color:"#555", marginBottom:"2.5rem", textAlign:"center" },
    input: { width:"100%", padding:"0.85rem", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:"4px", color:"#E8E0D0", fontSize:"1rem", fontFamily:"Georgia,serif", outline:"none", marginBottom:"0.75rem", boxSizing:"border-box" },
    btn: { width:"100%", padding:"1rem", background:"#C4A05A", border:"none", color:"#0F0F0E", fontSize:"0.85rem", fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer", fontFamily:"Georgia,serif", marginBottom:"0.5rem" },
    btnGhost: { width:"100%", padding:"1rem", background:"transparent", border:"1px solid rgba(196,160,90,0.3)", color:"#C4A05A", fontSize:"0.85rem", fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer", fontFamily:"Georgia,serif" },
    error: { fontSize:"0.75rem", color:"#D47070", marginBottom:"0.75rem" },
    code: { fontSize:"2.5rem", fontWeight:700, letterSpacing:"0.4em", color:"#C4A05A", textAlign:"center", padding:"1rem", background:"rgba(196,160,90,0.08)", border:"1px solid rgba(196,160,90,0.25)", borderRadius:"4px", marginBottom:"1rem" },
  };

  return (
    <div style={s.wrap}>
      <div style={{ width:"100%", maxWidth:380 }}>
        <div style={{ fontSize:"0.6rem", letterSpacing:"0.2em", color:"#C4A05A", textTransform:"uppercase", textAlign:"center", marginBottom:"0.5rem" }}>Elite Calisthenics Tour</div>
        {step === "choice" && (
          <>
            <div style={s.title}>Join the Competition</div>
            <div style={s.sub}>Create a club or join a friend's club to compete</div>
            <button style={s.btn} onClick={() => setStep("create")}>Create a Club</button>
            <button style={s.btnGhost} onClick={() => setStep("join")}>Join a Club</button>
          </>
        )}
        {step === "create" && (
          <>
            <div style={s.title}>Name Your Club</div>
            <div style={s.sub}>Choose something competitive</div>
            <input style={s.input} placeholder="e.g. The Peach State" value={clubName} onChange={e => setClubName(e.target.value)} onKeyDown={e => e.key === "Enter" && createClub()} />
            {error && <div style={s.error}>{error}</div>}
            <button style={s.btn} onClick={createClub} disabled={loading}>{loading ? "Creating..." : "Create Club"}</button>
            <button style={s.btnGhost} onClick={() => setStep("choice")}>Back</button>
          </>
        )}
        {step === "created" && (
          <>
            <div style={s.title}>Club Created!</div>
            <div style={s.sub}>Share this code with up to 5 friends</div>
            <div style={s.code}>{generatedCode}</div>
            <div style={{ fontSize:"0.7rem", color:"#555", textAlign:"center", marginBottom:"1.5rem" }}>Max 6 members — choose wisely</div>
            <button style={s.btn} onClick={onDone}>Start Playing</button>
          </>
        )}
        {step === "join" && (
          <>
            <div style={s.title}>Enter Invite Code</div>
            <div style={s.sub}>Get the 6-character code from your club founder</div>
            <input style={{...s.input, textTransform:"uppercase", letterSpacing:"0.3em", fontSize:"1.5rem", textAlign:"center"}} placeholder="ABC123" value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} maxLength={6} />
            {error && <div style={s.error}>{error}</div>}
            <button style={s.btn} onClick={joinClub} disabled={loading}>{loading ? "Joining..." : "Join Club"}</button>
            <button style={s.btnGhost} onClick={() => setStep("choice")}>Back</button>
          </>
        )}
      </div>
...
function ClubhousePage({ user, allRounds, allProfiles, allClubs }) {
  const [activeTab, setActiveTab] = useState("leaderboard");
  const [copied, setCopied] = useState(false);

  const myProfile = allProfiles?.find(p => p.id === user?.id);
  const myClub = allClubs?.find(c => c.id === myProfile?.clubId);

  if (!myClub) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"60vh", padding:"2rem", textAlign:"center" }}>
      <div style={{ fontSize:"2rem", marginBottom:"1rem" }}>club</div>
      <div style={{ fontSize:"1rem", fontWeight:700, marginBottom:"0.5rem" }}>No Club Yet</div>
      <div style={{ fontSize:"0.75rem", color:"#555" }}>Complete onboarding to join or create a club</div>
    </div>
  );

  const memberIds = myClub.memberIds || [];
  const members = memberIds.map(id => {
    const profile = allProfiles?.find(p => p.id === id);
    const memberRounds = (allRounds || []).filter(r => r.userId === id);
    const index = calcIndex(memberRounds);
    const recentRound = [...memberRounds].sort((a,b) => b.playedAt - a.playedAt)[0];
    return {
      id,
      name: profile?.name || profile?.email?.split("@")[0] || "Member",
      index,
      roundsPlayed: memberRounds.length,
      lastPlayed: recentRound?.playedAt || null,
      isYou: id === user?.id,
    };
  }).sort((a,b) => (a.index ?? 999) - (b.index ?? 999));

  const collectiveIndex = members.filter(m => m.index !== null).length
    ? members.filter(m => m.index !== null).reduce((s,m) => s + m.index, 0) / members.filter(m => m.index !== null).length
    : null;

  const spotsLeft = 6 - memberIds.length;

  function copyCode() {
    navigator.clipboard?.writeText(myClub.inviteCode || "");
    setCopied(true);
...
            <div style={{ fontSize:"1.4rem", fontWeight:700 }}>{myClub.clubName}</div>
            <div style={{ fontSize:"0.65rem", color:"#555", marginTop:"0.1rem" }}>{memberIds.length}/6 members</div>
          </div>
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:"0.55rem", color:"#555", textTransform:"uppercase", letterSpacing:"0.1em" }}>Club Index</div>
            <div style={{ fontSize:"1.8rem", fontWeight:700, color:scoreColor(collectiveIndex), lineHeight:1 }}>{fmtScore(collectiveIndex)}</div>
          </div>
        </div>
        {spotsLeft > 0 && (
          <div style={{ marginTop:"1rem", paddingTop:"0.75rem", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ fontSize:"0.6rem", color:"#555", marginBottom:"0.4rem" }}>Invite Code</div>
            <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
              <div style={{ fontSize:"1.5rem", fontWeight:700, letterSpacing:"0.3em", color:"#C4A05A" }}>{myClub.inviteCode}</div>
              <button onClick={copyCode} style={{ padding:"0.4rem 0.85rem", background:"rgba(196,160,90,0.15)", border:"1px solid rgba(196,160,90,0.3)", color:"#C4A05A", fontSize:"0.65rem", letterSpacing:"0.1em", textTransform:"uppercase", borderRadius:"3px", cursor:"pointer", fontFamily:"Georgia,serif" }}>
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
            <div style={{ fontSize:"0.6rem", color:"#444", marginTop:"0.25rem" }}>{spotsLeft} spot{spotsLeft !== 1 ? "s" : ""} remaining</div>
          </div>
        )}
      </div>

      <div style={{ display:"flex", marginBottom:"1.25rem", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
        {["leaderboard","club"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ flex:1, padding:"0.6rem 0", background:"none", border:"none", borderBottom:`2px solid ${activeTab===tab?"#C4A05A":"transparent"}`, color:activeTab===tab?"#C4A05A":"#555", fontSize:"0.65rem", letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", fontFamily:"Georgia,serif" }}>
            {tab}
          </button>
        ))}
      </div>
...
              <div style={{ fontSize:"0.7rem", color:"#444", width:"1.2rem", textAlign:"center", flexShrink:0 }}>{i+1}</div>
...
                </div>
                <div style={{ fontSize:"0.6rem", color:"#444", marginTop:"0.1rem" }}>
                  {member.roundsPlayed === 0 ? "No rounds yet" : `${member.roundsPlayed} round${member.roundsPlayed !== 1 ? "s" : ""}`}
                  {member.lastPlayed ? ` · ${fmtAgo(member.lastPlayed)}` : ""}
                </div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                {member.index !== null
                  ? <div style={{ fontSize:"1.1rem", fontWeight:700, color:scoreColor(member.index), lineHeight:1 }}>{fmtScore(member.index)}</div>
                  : <div style={{ fontSize:"0.7rem", color:"#444" }}>No index</div>}
              </div>
            </div>
          ))}
          {spotsLeft > 0 && (
            <div style={{ padding:"0.75rem", marginTop:"0.35rem", background:"rgba(255,255,255,0.01)", border:"1px dashed rgba(196,160,90,0.2)", borderRadius:"4px", textAlign:"center" }}>
              <div style={{ fontSize:"0.65rem", color:"#555" }}>{spotsLeft} open spot{spotsLeft !== 1 ? "s" : ""}</div>
              <div style={{ fontSize:"0.6rem", color:"#444", marginTop:"0.15rem" }}>Share code <span style={{ color:"#C4A05A", letterSpacing:"0.1em" }}>{myClub.inviteCode}</span> to invite</div>
            </div>
          )}
        </div>
      )}
...
            {[
              { label:"Club Index", value:fmtScore(collectiveIndex), color:scoreColor(collectiveIndex) },
              { label:"Members", value:`${memberIds.length}/6`, color:"#E8E0D0" },
              { label:"Total Rounds", value:members.reduce((s,m)=>s+m.roundsPlayed,0), color:"#E8E0D0" },
              { label:"Invite Code", value:myClub.inviteCode, color:"#C4A05A" },
            ].map(stat => (
...
          {Object.entries(COURSES).map(([key, c]) => {
            const courseRounds = (allRounds || []).filter(r => memberIds.includes(r.userId) && r.courseKey === key);
            if (!courseRounds.length) return (
              <div key={key} style={{ padding:"0.75rem", marginBottom:"0.35rem", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:"4px" }}>
                <div style={{ fontSize:"0.82rem", fontWeight:700 }}>{c.name}</div>
                <div style={{ fontSize:"0.6rem", color:"#555" }}>Slope {c.slope} · No rounds yet</div>
              </div>
            );
            const best = courseRounds.reduce((b,r) => r.finalScore < (b?.finalScore ?? Infinity) ? r : b, null);
            const holder = allProfiles?.find(p => p.id === best?.userId);
            return (
              <div key={key} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"0.75rem", marginBottom:"0.35rem", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:"4px" }}>
                <div>
                  <div style={{ fontSize:"0.82rem", fontWeight:700 }}>{c.name}</div>
                  <div style={{ fontSize:"0.6rem", color:"#555" }}>Slope {c.slope} · {holder?.name || "Unknown"}</div>
                </div>
                <div style={{ fontSize:"1.1rem", fontWeight:700, color:scoreColor(best?.finalScore) }}>{fmtScore(best?.finalScore)}</div>
              </div>
            );
          })}
          <div style={{ marginTop:"2rem", paddingTop:"1rem", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
            <button onClick={copyCode} style={{ width:"100%", padding:"0.75rem", background:"transparent", border:"1px solid rgba(196,160,90,0.3)", color:"#C4A05A", fontSize:"0.75rem", letterSpacing:"0.1em", textTransform:"uppercase", borderRadius:"4px", cursor:"pointer", fontFamily:"Georgia,serif" }}>
...
          <div style={{fontSize:"1.1rem"}}>{item.icon==="golf"?"⛳":item.icon==="stats"?"📊":"🏛️"}</div>
...
  const { data } = db.useQuery(user ? { rounds: {}, profiles: {}, clubs: {} } : null);
  const allRounds = data?.rounds || [];
  const allProfiles = data?.profiles || [];
  const allClubs = data?.clubs || [];
  const myRounds = allRounds.filter(r => r.userId === user?.id).sort((a,b) => b.playedAt - a.playedAt);
  const myProfile = allProfiles.find(p => p.id === user?.id);
  const hasClub = !!(myProfile?.clubId);
...
      {page==="club" && <ClubhousePage user={user} allRounds={allRounds} allProfiles={allProfiles} allClubs={allClubs}/>}
